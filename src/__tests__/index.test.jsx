...
test("1.3.1 Task'i 4 kişiye atayınca hata mesajı doğru metinle çıkıyor.", async () => {
  const user = userEvent.setup();
...
test("1.3.2 Task'i 1 kişiye atayıp, o kişi çıkarılınca hata mesajı doğru metin ile çıkıyor.", async () => {
  const user = userEvent.setup();
  const assignee = screen.getByLabelText(/İsim/i);
  const addButton = screen.getByText('Ekle');
  await user.type(assignee, 'Erdem');
...
test('3.3 Tamamlandı butonuna tıklayınca "2 id\'li görev tamamlandı." mesajı toastify ile çıkıyor.', async () => {
  const user = userEvent.setup();
  const button = screen.getAllByText('Tamamlandı');
  await user.click(button[0]);
  const tamamlanmamisGorevler = await screen.findByText(
    "2 id'li görev tamamlandı."
  );
});
